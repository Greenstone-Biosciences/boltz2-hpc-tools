	...
	do
		...
		if [[ ! -f ${Input}/Ligand_yamls/boltz_results_${name}_affinity/predictions/*/affinity*affinty.json ]]; then
			...
			if [ -n "$USE_MSA" ]; then
				...
				yq --arg seq "$sequence" --arg msa "$USE_MSA" --arg smi "$smile" '.sequences[0].protein.sequence = $seq | .sequences[0].protein.msa = $msa | .sequences[1].ligand.smiles = $smi' /data/Shared/boltz/affinity.yaml > ${Input}/Ligand_yamls/${name}_affinity.yaml;
			else
				echo "no MSA provided, using server"
				yq --arg seq "$sequence" --arg smi "$smile" '.sequences[0].protein.sequence = $seq | del(.sequences[0].protein.msa) | .sequences[1].ligand.smiles = $smi' /data/Shared/boltz/affinity_template.yaml > ${Input}/Ligand_yamls/${name}_affinity.yaml;
			fi
