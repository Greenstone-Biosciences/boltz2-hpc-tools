...
if [[ ! -d "$InputDIR"/Ligand_yamls ]]; then 
	...
	fileNUM=$(find $InputDIR/Ligand_yamls -name "affinity*affinity.json" | wc -l)
	if [ $fileNUM -eq 0 ]; then 
		echo "Found the directory ${InputDIR}/Ligand_yamls, but it appears to be empty!"
		exit 1
	else
		echo "Found ${fileNUM} files!  Processing now..."
		first_file=$(find $InputDIR/Ligand_yamls -name "affinity*affinity.json" | head -n 1)		
		...
		#Process the files
		find $InputDIR/Ligand_yamls/ -name "affinity*affinity.json" | while read -r file; 
		do 
