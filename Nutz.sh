...
do
	...
	if sed '2q;d' $working_file | grep -q ',' ; then 
		smile=$(echo $line | cut -d ',' -f1 | tr -d '\r\n')
		name=$(echo $line | cut -d ',' -f2 | tr -d '\r\n')
	else
		smile=$(echo $line | awk '{ print $1 }' | tr -d '\r\n');
	...
	if [ -n "$USE_MSA" ]; then
		...
		echo "Saving file to ${Input}/Nut_${padded_id}/Ligand_yamls"
		yq --arg seq "$sequence" --arg msa "$USE_MSA" --arg smi "$smile" '.sequences[0].protein.sequence = $seq | .sequences[0].protein.msa = $msa | .sequences[1].ligand.smiles = $smi' AFFINITY_TEMPLATE > ${Input}/Ligand_yamls/${name}_affinity.yaml;
	else
		echo "no MSA provided, using server"
		yq --arg seq "$sequence" --arg smi "$smile" '.sequences[0].protein.sequence = $seq | del(.sequences[0].protein.msa) | .sequences[1].ligand.smiles = $smi' AFFINITY_TEMPLATE > ${Input}/Ligand_yamls/${name}_affinity.yaml;
	fi
done
	...
	if [ -n "$USE_MSA" ]; then
		...
		echo "no MSA provided, using server"
		boltz predict ./Ligand_yamls --use_msa_server
	fi
